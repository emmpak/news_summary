  //
  // ArticleList.prototype.saveArticles = function(input) {
  //   for( var i = 0; i < input.response.results.length; i ++) {
  //     var summary = getSummary("I am a summary!");
  ...
  (function() {
    var articleList = new ArticleList()
    articleList.saveArticles(response())
    assert.doesInclude(articleList.articles[0].summary, "north-east regional chairman, Steve Turner, has defected to the Conservative party after saying that the anti-EU party was riddled with “infighting and poor organisation”.")
  })();
